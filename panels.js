(function(){
  ...
  const tabs = { bio: q('tabBio'), visuals: q('tabVisuals'), eco: q('tabEco'), draw: q('tabDraw') };
  const panels = { bio: q('bioPanel'), visuals: q('visualsPanel'), eco: q('ecoPanel'), draw: q('drawPanel') };
  function setActiveTab(name){
    ...
    if (window.appSetMode){
      const isDraw = (name === 'draw' || name === 'eco');
      window.appSetMode(isDraw ? 'draw' : 'preset');
    }
  }
  ...
  if (tabs.eco) tabs.eco.addEventListener('click', ()=> setActiveTab('eco'));
